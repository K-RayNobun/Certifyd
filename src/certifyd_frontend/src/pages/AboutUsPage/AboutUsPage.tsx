    ...
    <div className="min-h-screen bg-white font-Outfit selection:bg-[#0066FF] selection:text-white pb-32">
      ...
      <section className="bg-[#0A2540] py-32 px-8">
         ...
         <div className="max-w-6xl mx-auto bg-white rounded-[60px] p-8 md:p-12 shadow-[0_40px_100px_rgba(0,102,255,0.2)] flex flex-col md:flex-row items-center gap-12 relative overflow-hidden">
            ...
            <div className="md:w-1/2 relative z-10 px-6">
               ...
               <button className="py-5 px-10 bg-[#0066FF] hover:bg-[#0A2540] text-white rounded-full font-black text-[10px] tracking-[4px] uppercase transition-all shadow-2xl">
                  Watch Demo
               </button>
            </div>

            <div className="md:w-1/2 relative z-10 bg-gray-50 rounded-[40px] h-[400px] w-full flex items-center justify-center border-4 border-white shadow-inner group overflow-hidden cursor-pointer">
               <div className="absolute inset-0 bg-[#0A2540] opacity-0 group-hover:opacity-10 transition-opacity"></div>
               <div className="w-24 h-24 bg-[#0A2540] text-white rounded-full flex items-center justify-center text-3xl shadow-4xl transform group-hover:scale-110 transition-transform absolute z-20">
                  ▶
               </div>
               {/* Decorative Video Mock */}
               <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-48 h-48 bg-gray-200 rounded-[40px] shadow-2xl flex items-center justify-center text-5xl transform group-hover:translate-y-4 transition-all duration-700">
                  🤖
               </div>
            </div>
         </div>
      ...
      <section className="py-32 px-8 text-center max-w-6xl mx-auto">
         ...
         <div className="flex flex-wrap justify-center items-center gap-12 lg:gap-24 opacity-40 grayscale pt-8">
            {['Inpulsion', 'Logipsum', 'LOGO IPSUM', 'BOGO', 'Logipsum', 'Inpulsion', 'Logipsum', 'LOGO IPSUM', 'Logipsum', 'LOGO IPSUM'].map((partner, idx) => (
               <div key={idx} className="text-2xl font-black uppercase tracking-widest text-[#0A2540] hover:text-[#0066FF] hover:opacity-100 hover:grayscale-0 transition-all cursor-pointer">
                  {partner}
               </div>
            ))}
         </div>
      ...
      <footer className="pt-20 px-8 lg:px-24 border-t border-gray-100 max-w-7xl mx-auto flex flex-col md:flex-row justify-between gap-12">
         <div className="max-w-sm">
            <h3 className="text-3xl font-black text-[#0A2540] tracking-tighter mb-4">Certifyd Protocol</h3>
         ...
         <div className="flex gap-20">
            <div>
               ...
               <ul className="space-y-4 text-xs font-bold text-gray-400">
                  <li className="hover:text-[#0066FF] cursor-pointer transition-colors">Students</li>
                  <li className="hover:text-[#0066FF] cursor-pointer transition-colors">Institutions</li>
               </ul>
            </div>
            <div>
               <h5 className="text-[10px] font-black uppercase tracking-[4px] text-[#0A2540] mb-6">Protocol</h5>
               <ul className="space-y-4 text-xs font-bold text-gray-400">
                  <li className="hover:text-[#0066FF] cursor-pointer transition-colors">FAQ</li>
                  <li className="hover:text-[#0066FF] cursor-pointer transition-colors">Smart Contract</li>
               </ul>
            </div>
         </div>
         ...
         <div>
            ...
            <div className="flex bg-gray-50 rounded-full p-2 border border-gray-200 focus-within:border-[#0066FF] transition-colors shadow-inner">
               <input type="email" placeholder="Enter your email address" className="bg-transparent pl-4 text-xs font-bold w-full outline-none text-[#0A2540]" />
               <button className="w-10 h-10 bg-[#0A2540] hover:bg-[#0066FF] text-white rounded-full flex items-center justify-center transition-colors font-black shadow-lg">
                  &rarr;
               </button>
            </div>
         </div>
